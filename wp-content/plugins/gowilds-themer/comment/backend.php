<?php
class Gowilds_Listing_Comment_BE extends Gowilds_Listing_Comment{
	...
	public function add_meta_box( $comment ) {
		if ( ! current_user_can( 'edit_comment', $comment->comment_ID ) || 'trip' !== get_post_type( $comment->comment_post_ID ) || 0 !== intval( $comment->comment_parent ) ) {
			return;
		}
		add_meta_box(
			$id         = 'lt_review',
			$title      = __( 'Review', 'gowilds-themer' ),
	...
	public function output_html( $comment, $box ) {
		...
		<table class="form-table">
			...
			<tr><td colspan="2"><h3><?php echo esc_html__('Category review was deleted', 'gowilds-themer') ?></h3><td></tr>
			<?php foreach ( $categories_star as $cat ) :
				...
				<tr>
				  	...
				  	<td> 
					  	<select name="lt_review[<?php echo $cat ?>]">
							...
							<?php endfor; ?>
					 		<option value="delete"><?php echo esc_html__('Delete', 'gowilds-themer') ?></option>
					 	</select>
			 ...
			 <?php wp_nonce_field( 'gowilds_lt_save_data', 'gowilds_lt_meta_nonce' ); ?>
		</table>
	<?php
	}
	...
	public function save_comment( $comment_id, $data ) {
		...
		if ( empty( $_POST['gowilds_lt_meta_nonce'] ) || ! wp_verify_nonce( $_POST['gowilds_lt_meta_nonce'], 'gowilds_lt_save_data' ) ) {
			return $comment_id;
		}
		if ( 'trip' !== $post->post_type || 0 !== intval( $data['comment_parent'] ) ) {
			return $comment_id;
		}
...
new Gowilds_Listing_Comment_BE();