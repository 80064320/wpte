...
class Gowilds_Listing_Comment{
   ...
   public function categories_review(){
      $cat_reivews = gowilds_themer_get_theme_option('lt_reviews');
      $results = array();
      if( empty($cat_reivews) ){
         $cat_reivews = array('Quality[quality]', 'Location[location]', 'Services[services]', 'Pricing[price]');
      } 
      ...
      $ouput .= '<div class="lt-review-show-start">';
         ...
         $ouput .= '<div class="review-results">';
            $ouput .= '<div class="base-stars">';
              for ($i=1; $i <= 5; $i++) { 
                  $ouput .= '<span><i class="fas fa-star"></i></span>';
               }
            $ouput .= '</div>';
