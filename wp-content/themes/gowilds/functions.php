...
add_action('after_setup_theme', 'gowilds_init');
function gowilds_init(){
	if(class_exists('Redux')){
		...
		if(class_exists('\WPTravelEngine\Plugin')){ 
			require_once(GOWILDS_THEME_DIR . '/includes/options/opts-trip-general.php'); 
		}
		if(class_exists('WooCommerce')){
...
function gowilds_init_scripts(){
	...
	//WP Travel Engine
	if(class_exists('\WPTravelEngine\Plugin')){ 
		wp_enqueue_style('gowilds-booking-wte', GOWILDS_THEME_URL . '/assets/css/booking-wte.css', array(), $theme_version , 'all');
	}
...
function gowilds_modify_taxonomy() {
   if(taxonomy_exists('ba_location')){
      $tax_location_args = get_taxonomy('ba_location');
      if($tax_location_args){
         if(isset($tax_location_args->rewrite['slug'])){
            $tax_location_args->rewrite['slug'] = 'location';
            register_taxonomy( 'ba_location', 'to_book', (array)$tax_location_args);
         }
     }
   }
}
add_action( 'init', 'gowilds_modify_taxonomy', 11 );