	...
	<div class="post-data itinerary wte-trip-itinerary-v2">
		...
		foreach ( $itineraries as $key => $value ) :
			...
			<div class="itinerary-row <?php echo( esc_attr( $active_check ) ); ?>">
				<div class="wte-itinerary-head-wrap">
					<div class="title">
						...
						if ( isset( $tabs[ 'itinerary' ][ 'itinerary_days_label' ][ $key ] ) && ! empty( $tabs[ 'itinerary' ][ 'itinerary_days_label' ][ $key ] ) ) {
							...
							printf( esc_html__( 'Day %s :', 'gowilds' ), esc_attr( $key ) );
						}
						?>
					</div>
					...
					<?php if ( ! empty( $inner_content[ $key ] ) ) : ?>
						<span
							class="dashicons dashicons-arrow-down custom-toggle-tabs rotator <?php echo esc_attr($enabled_expand_all ? 'open' : ''); ?>"></span>
					<?php endif; ?>
				...
				<div class="itinerary-content <?php echo esc_attr($key == 1 ? 'show' : ''); ?>">
